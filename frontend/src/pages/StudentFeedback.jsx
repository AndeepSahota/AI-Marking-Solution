...
import { getLesson, getLessonQuestions, getStudents, getMarkingResults } from '../services/api'
...
function hasDescriptorEvidence(breakdown) {
  return (breakdown ?? []).some(criterion =>
    (criterion.evidenceSupportingAwardedBand ?? [])
      .some(descriptor => Array.isArray(descriptor.evidence) && descriptor.evidence.length > 0)
  )
}
...
function StudentFeedback() {
  ...
  useEffect(() => {
    ...
    // Marking (today) always marks the FIRST selected question index, not
    // every selected one — matches resolveSelectedQuestions(ocrRow)[0] on
    // the backend. Only that one question's descriptors are relevant here.
    getLessonQuestions(lessonId)
      .then(data => {
        const idx = data.selected_question_indices?.[0] ?? 0
        setScheme(data.questions?.[idx] ?? null)
      })
              ...
              <div key={r.studentId} className="student-marking-row">
                ...
                {isExpanded && (
                  <div className="student-result-expanded">
                    <ResultCard result={r.result} scheme={scheme} />
