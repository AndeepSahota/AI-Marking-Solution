...
// declarative <BrowserRouter>, where that prop is silently a no-op.
// document.startViewTransition() is a native browser API though — nothing
// about calling it manually requires the data router. flushSync is required
// here: startViewTransition's callback needs the DOM update to happen
// synchronously so it can capture the "after" snapshot — a normal (batched,
// async) navigate() would race it.
            ...
            <div className="header-inner">
                ...
                {user && (
                    <nav className="header-nav">
                        {/* Scoped to just these persistent nav destinations — every other
                            navigation (Home into a marking session, back buttons, etc.)
                            is process flow, not nav, and stays untransitioned. */}
                        ...
                        <NavLink to="/exemplars" className="header-nav-link" onClick={handleNavClick('/exemplars')}>Exemplars</NavLink>
                    </nav>
                )}
