...
class DescriptorValidationTests(unittest.TestCase):
    ...
    def test_prose_dash_after_a_question_mark_is_not_treated_as_a_bullet(self):
        # Real AQA Q1 answer-key style: "question? - answer" pairs, where the
        # dash is ordinary punctuation joining a question to its answer, not
        # a list marker. Regression for a bug where this dash was matched as
        # a bullet, splitting each Q&A pair apart and bleeding content across
        # separate questions.
        self.assertEqual(
            split_descriptor_bullets(
                "What does this suggest about the setting? - It suggests a "
                "sense of isolation. What does the writer's use of pathetic "
                "fallacy add? - It reinforces the character's mood."
            ),
            [
                "What does this suggest about the setting? - It suggests a sense of isolation.",
                "What does the writer's use of pathetic fallacy add? - It reinforces the character's mood.",
            ],
        )
    ...
    def test_content_integrity_accepts_flattened_raw_descriptor(self):
        ...
        # No bullet markers at all — the boundary checker falls back to
        # sentence-boundary splitting, which agrees with the model's split
        # here. (It used to report a mismatch instead: main.py raises on the
        # first shape issue before content-integrity is ever consulted, so
        # that mismatch blocked every prose-style mark scheme outright — this
        # case is exactly the pattern that surfaced that in production.)
        self.assertEqual(validate_descriptor_shapes(source), [])
        self.assertEqual(validate_descriptor_content_integrity(source), [])
