...
export const lessonDb = {
    ...
    // Used by bulk-mark to fetch the whole class roster for a lesson.
    getClassId: async (lessonId, teacherId, e = pool) =>
        (await exec(e, `
            SELECT l.class_id
            FROM dbo.lessons AS l
...
// Least-privilege accessor for the teacher feedback loop — not wired into any
// route yet. Captures every accept/override decision so accuracy can
// eventually be measured against real teacher judgement (Cohen's Kappa),
// rather than assumed.
export const markCorrectionsDb = {
    insert: async (resultId, teacherId, llmScore, teacherCorrection, newScore, e = pool) =>
        (await exec(e, `
            INSERT INTO dbo.mark_corrections (result_id, teacher_id, llm_score, teacher_correction, new_score)
            OUTPUT INSERTED.id
            VALUES (@resultId, @teacherId, @llmScore, @teacherCorrection, @newScore)
        `, [
            intParam('resultId', resultId),
            intParam('teacherId', teacherId),
            intParam('llmScore', llmScore),
